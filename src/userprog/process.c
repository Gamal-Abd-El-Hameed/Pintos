...
process_execute (const char *file_name) 
{
  ...
  /* Make a second copy of FILE_NAME. */
  ...
  if(thread_current()->child_process_creation)
    return tid;
  else
    return TID_ERROR;
}
...
start_process (void *file_name_)
{
  ...
  const char **prog_args = (char **) palloc_get_page(0);
  ...
  if (prog_args != NULL)
  {
    ...
    if (success)
      push_arguments (prog_args, cnt, &if_.esp);
    palloc_free_page(prog_args);

    thread_current()->parent_thread->child_process_creation = success;
  }
  else
    thread_current()->parent_thread->child_process_creation = false;
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  for (e = list_begin (&thread_current()->child_processes); e != list_end (&thread_current()->child_processes);
       e = list_next (e))
  {
    struct child_process *f = list_entry (e, struct child_process, child_elem);
    if(f->pid == child_tid)
    {
      child = f;
      child_list_elem = e;
  ...
  if(thread_current()->waiting_on != -1)
    return -1;
  else
    thread_current()->waiting_on = child->pid;

  if(thread_current()->child_status == -100)
  {
    sema_up(&child->t->parent_child_sync);
    sema_down(&thread_current()->wait_child);
  }
  list_remove(child_list_elem);
...
process_exit (void)
{
  ...
  struct list *child_process_list = &cur->child_processes;
  struct list_elem *e;

  for (e = list_begin (child_process_list); e != list_end (child_process_list);
       e = list_next (e))
  {
    struct child_process *child_process = list_entry(e, struct child_process, child_elem);
    struct thread *child_thread = child_process->t;
